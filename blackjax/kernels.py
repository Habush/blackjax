...
class dmala:
    ...
    The general mala kernel (:meth:`blackjax.mcmc.dmala.kernel`, alias `blackjax.dmala.kernel`) can be
    cumbersome to manipulate. Since most users only need to specify the kernel
    parameters at initialization time, we provide a helper function that
    specializes the general kernel.
    ...
    .. code::
        ...
        dmala = blackjax.dmala(logprob_fn, step_size)
    ...
    .. code::
       ...
       kernel = blackjax.dmala.kernel(logprob_fn)
       state = blackjax.dmala.init(position, logprob_fn)
       state, info = kernel(rng_key, state, logprob_fn, step_size)
...
class mixed_mala:
    ...
    init = staticmethod(mcmc.mixed_mala.init)
    ...
    def __new__(cls,
        ...
        def init_fn(position: PyTree):
            ...
            return cls.init(disc_position, contin_position,
...
class sgdld:
    ...
    init = staticmethod(sgmcmc.sgld.init)
    kernel = staticmethod(sgmcmc.sgdld.kernel)
